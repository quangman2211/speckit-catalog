...
| `phases` | catalog `quangman` | Skill theo giai doan spec/plan + hook goi y |

## Yeu cau truoc khi cai

Extension `phases` khong ship kem specify-cli, nen catalog phai duoc dang ky
...
specify integration upgrade claude    # noi hook `events:`
```

## Cai tu artifact da build

```bash
specify bundle install dist/retail-frontend-1.2.0.zip
```
